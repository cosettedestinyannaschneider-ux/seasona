...
管理员账号不提供公开注册接口。需要手动新增管理员时，先在项目环境中运行：

```bash
python scripts/hash_password.py
```

脚本会交互式要求输入并确认密码，然后输出符合项目 Argon2id 配置的哈希值。它不连接数据库，只负责生成可写入 `user_account.password_hash` 的字符串。随后可在数据库中手动插入管理员账号，`role` 使用 `admin`，`status` 使用 `active`，密码字段填入脚本输出的完整 `$argon2id$...` 字符串。
